...
SOURCE_URL = "http://buzzlog.yahoo.com/feeds/buzzoverl.xml"
SOURCE_NAME = 'hott_rends'.strip().lower().replace(' ', '_')
...
def parse_feeds(data):
    ...
    hot_query_links = soup.findAll('title', type=re.compile("html"))
    print 'total:', len(hot_query_links)/2
    for q in hot_query_links:
        if q.text != 'Yahoo! Buzz Index Leaders Overall':
            ctext = q.text.split('.')[1].strip()
            print ctext
            feeds.append(ctext)
